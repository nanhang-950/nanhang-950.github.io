...
### 虚拟文件与file_operations

什么是虚拟文件系统（VFS）

- **VFS（Virtual File System）** 是 Linux 内核中一个用于统一管理多种不同文件系统（ext4、tmpfs、procfs 等）的 **抽象层**。
- 提供统一的 API 接口，使用户空间不必关心底层文件系统的具体实现。
- 所有文件、目录、设备文件的操作最终都会调用 VFS 中定义的一组通用方法（如 `open`、`read`、`write` 等）。

`file_operations`与虚拟文件系统的关系：

- `file_operations` 是内核中定义的一个结构体，表示一个“文件对象”支持的操作集合。
- 每个设备文件或虚拟文件都可以实现自己的 `file_operations`，并注册到内核中。
- 当用户执行如 `open("/dev/xxx")`、`read()`、`ioctl()` 等操作时，最终会触发对应的 `file_operations` 中的函数指针调用。
...
内核模块的初始化函数在编译时通过`module_init()`定义，在内核模块被载入时会调用所定义的函数，这里我们将初始化函数设定为`kernel_module_init`。内核模块的卸载函数在编译时通过`module_exit`定义，在内核模块被卸载时会调用所定义的函数，这里我们将卸载函数定义为`kernel_module_exit`。
...
我们需要通过分析`core.ko`中的漏洞来编写相应的 exp 进行利用，和用户态的 pwn 不同的是内核的 exp 一般通过 C 语言编写，程序通过模块定义的函数来与内核交互，利用方式就是将 C 编译好的 exp 丢到模拟内核的虚拟机上运行。
...
先解包`core.cpio`，`file`查看发现`core.cpio`文件是经过 gzip 压缩过的，先解压然后再解包。
...
参数：`-s -S`等价于`-gdb tcp::1234`，监听 1234 端口。

gdb 调试步骤：

`vmlinux`是 Linux 内核的未压缩 ELF 可执行文件，我们使用 gdb 调试内核主要就是调试这个文件。

```shell
gdb vmlinux            
(gdb) target remote :1234                        #attach 到目标端口
(gdb) set architecture aarch64                   #如果是异架构，则设置对应架构
```

在有些题目中会直接给我们一个 bzImage 文件，这是不能用于调试的。但是我们可以通过[vmlinux-to-elf]([marin-m/vmlinux-to-elf：用于恢复完全可分析的 .ELF 来自原始内核，通过提取内核符号表 （kallsyms）](https://github.com/marin-m/vmlinux-to-elf))工具将其提取为带符号可调式的 vmlinux 文件：
...
(gdb) x/32gx <addr>
(gdb) info threads
(gdb) thread apply all bt full #对当前进程中的每个线程都执行 bt full
```
...
(gdb) add-symbol-file vuln.ko 0xffffffffc0000000 #加载内核模块符号
```
...
```shell
cat /proc/modules     # 可得内核模块加载基址
cat /proc/version     # 查看当前运行的内核版本
```
